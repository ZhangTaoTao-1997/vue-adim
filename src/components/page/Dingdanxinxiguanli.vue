  ...
  <div class="table">
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item><i class="el-icon-lx-cascades"></i> 订单信息管理</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="container">
      ...
      <h4 style="color: #67c23a">出货总金额:{{totalNumber}} 元</h4>
      <!--分页的实现-->
    ...
    <!--入库明细弹出框 -->
    <el-dialog title="出库明细" :visible.sync="editVisible" width="50%">
      <el-table
        :data="Ruku"
        border
        style="width: 100%">
    ...
    <!-- 新增提示框 -->
    <el-dialog title="添加出库明细" :visible.sync="addVisible" width="30%">
...
//      },
      ...
      info() {
        this.$axios({
          type: "get",
          url:'http://127.0.0.1:9202/chuku/findAll',
          dataType: 'json',
        }).then(res=>{
          this.Tuser=res.data
          //这些数据视情况，是你从后端拿来的。
      ...
      handleEdit(index, row) {
        ...
        that.$axios({
          type: "post",
          url:'http://127.0.0.1:9202/chukumingxi/findAll?id='+that.id,
          dataType: 'json',
        }).then(res=>{
          that.Ruku=res.data
...
//      },
      ...
      deleteRow(row) {
        ...
        data.push(row.id);
        that.$axios({
          url: 'http://127.0.0.1:9202/chuku/delete',
          method: "post",
          data: data
        }).then(res => {
          if (res.data == 1) {
      ...
      deleteRow1(row) {
        ...
        data.push(row.id);
        that.$axios({
          url: 'http://127.0.0.1:9202/chukumingxi/del1ete',
          method: "post",
          data: data
        }).then(res => {
          if (res.data == 1) {
      ...
      addEdit(){
        var that = this
        let data = {
          chukuId:that.form.id,  //此title为 input 输入框的内容
          goodsId:that.form1.goodsId,
          shuliang:that.form1.shuliang,
          beizhu:that.form1.beizhu,
        };
        that.$axios({
          url: 'http://127.0.0.1:9202/chukumingxi/addChukumingxi',
          method: 'post',
          data: data
        }).then(res => {
          if (res.data == 1) {
